...
import qualified LocalComputation.Instances.ShortestPath.SingleTarget as S

type DistanceGraph a b = [[((a, a), b)]]

create :: S.Graph a b -> G.Graph a b
create xs = G.Graph $ M.map (map (\(destination, cost) -> G.EdgeTo destination cost)) xs
