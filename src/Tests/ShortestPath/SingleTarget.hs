...
prop_p1fusion = unitTest $ do
    result <- pure $ fromRight $ ST.singleTarget' [p1Graph] (head $ p1Queries.sources) p1Queries.target
...
prop_p2fusion = unitTest $ do
    result <- pure $ fromRight $ ST.singleTarget' p2Graph (head $ p2Queries.sources) p2Queries.target
...
matchesPrebuiltFusion g numTests = withTests numTests . withDiscards 10000 . property $ do
    ...
    let prebuiltResults =             H.singleTarget g ([head $ query.sources]) query.target infinity
    if all (== infinity) prebuiltResults then discard else pure ()
    result <- pure $ fromRight $ ST.singleTarget' [(fmap T g)] (head $ query.sources) query.target
